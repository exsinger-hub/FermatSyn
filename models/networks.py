...
def define_G(
    ...
    elif which_model_netG == "mamba":
        print(vit_name)
        netG = modules.ABS_Mamba(
            modules.CONFIGS[vit_name],
            input_dim=input_nc,
            img_size=img_size,
            output_dim=1,
...
def define_D(
    ...
    if which_model_netD == "basic":
        ...
        netD = modules.Transformer_Discriminator(
            modules.CONFIGS[vit_name],
            img_size=img_size,
            output_dim=1,
            zero_head=False,
...
# When LSGAN is used, it is basically the same as MSELoss,
...
class Encoder_Decoder(nn.Module):
    def __init__(
        ...
        for ii in range(2):
            ...
            model += [
                nn.Conv2d(ngf * mult, latent_size, kernel_size=3, padding=1),
                norm_layer(latent_size),
                nn.ReLU(True),
            ]
        ...
        model = [
            nn.Conv2d(latent_size * 2, 256, kernel_size=3, padding=1),
            norm_layer(256),
            nn.ReLU(True),
        ]
        ### upsample
...
class PixelDiscriminator(nn.Module):
    def __init__(
        self, input_nc, ndf=64, norm_layer=nn.BatchNorm2d, use_sigmoid=False, gpu_ids=[]
    ):
